...
def runtime_nx(v, e):
    ...
    our_graph_algo.load_from_json("../data/G_{}_{}_1.json".format(v,e))
    end_time = time.time()
    ...
    print("end load graph G_{}_{}_1   our time : {}    networkx time : {}".format(v,e,our_time_load, nx_time_load))
    start_time = time.time()
    ...
    print("""end connected components algorithm on graph G_{}_{}_1  our time : {}   networkx time : {}""".format(
    ...
    print("""end shortest_path algorithm on graph G_{}_{}_1 our time : {}  networkx time : {}""".format(
...
def runtime(v,e):
    ...
    print("end load graph G_{}_{}_1    time : {} ".format(v, e, time_load,))
    graph = graph_algo.get_graph()
    start_time = time.time()
    graph_algo.connected_component(0)
    end_time = time.time()
    time_component = end_time - start_time
    print("""end connected component algorithm for node 0  on graph G_{}_{}_1 time : {}  """.format(
        v, e, time_component))
    start_time = time.time()
    graph_algo.connected_components()
    end_time = time.time()
    time_components = end_time - start_time
    print("""end connected components algorithm on graph G_{}_{}_1 time : {}  """.format(
        v, e, time_components,))
    start_time = time.time()
    shortest_path = graph_algo.shortest_path(0, graph.v_size() - 1)
    end_time = time.time()
    time_shortest_path = end_time - start_time
    print("""end shortest_path algorithm on graph G_{}_{}_1  time : {}  """.format(
